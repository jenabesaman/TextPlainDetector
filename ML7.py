...
random_strings_complex = [''.join(random.choices(string.ascii_letters + string.digits + '@#%&*', k=15)) for _ in range(len(dictionary_words)//2)]
...
random_strings_simple = [''.join(random.choices(string.ascii_letters, k=5)) for _ in range(len(dictionary_words)//2)]
...
all_words = dictionary_words + random_strings_complex + random_strings_simple

# Create labels for the words (True for dictionary words, False for random strings)
labels = [True] * len(dictionary_words) + [False] * len(random_strings_complex) + [False] * len(random_strings_simple)
