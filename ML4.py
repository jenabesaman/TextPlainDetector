...
def read_file_as_text(file_path, encodings):
    try:
        # Try to read the file with 'latin-1' encoding first
        with open(file_path, 'rb') as file:
            return file.read().decode('latin-1')
    except UnicodeDecodeError:
        # If 'latin-1' fails, try the other encodings
...
texts = [read_file_as_text(file_path, ['latin-1', 'utf-8', 'windows-1256']) for file_path in file_paths]
